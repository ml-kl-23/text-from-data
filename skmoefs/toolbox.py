...
import numpy as np
from Platypus.platypus import AdaptiveGridArchive, Problem
...
def load_dataset(name):
    ...
    #with open('dataset/' + name , 'r') as f:  #with function
        line = f.readline()
        while line:
            if line.startswith("@"):
                txt = line.split()
...
class MPAES_RCS(MOEL_FRBC):
    ...
    def show_model(self, position='first', inputs=None, outputs=None, f=None):
        ...
        if index is not None:
            a = self.classifiers[index].show_RB(inputs, outputs, f)
            b = self.classifiers[index].show_DB(inputs)
